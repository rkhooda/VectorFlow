...
Implements the contract described in `docs/integration.md` using the Pydantic
models from `contracts/`. The backend imports this package in-process when
`config.yaml` sets `modules.data_pipeline.implementation: real` (the default);
`mock` falls back to `backend/app/services/mocks.py`.
